...
impl BookReader for EpubReader {
    ...
    fn chapter_blocks(&self, chapter_idx: usize) -> Result<Vec<ContentBlock>> {
        ...
        let text = html2text::from_read(processed_html.as_bytes(), 80)
            .unwrap_or_else(|_| {
...
fn extract_img_tags(html: &str) -> Vec<(String, String)> {
    ...
    let mut pos = 0;
    while pos < html.len() {
        let Some(rel_start) = html_lower[pos..].find("<img") else { break };
        let abs_start = pos + rel_start;
        ...
        let Some(rel_end) = html_lower[abs_start..].find('>') else { break };
    ...
    while pos < tag_lower.len() {
        ...
        if prev_ok {
            ...
            return if rest.starts_with('"') {
                rest[1..].find('"').and_then(|e| rest.get(1..1 + e)).map(str::to_string)
...
fn resolve_href(chapter_href: &str, img_src: &str) -> String {
    ...
    let base_dir = chapter_href.rfind('/').map(|i| &chapter_href[..=i]).unwrap_or("");
...
mod tests {
    ...
    fn img_tags_basic() {
        ...
        assert_eq!(imgs, vec![("images/fig.png".to_string(), "Figure 1".to_string())]);
    }
    ...
    fn resolve_external_skipped() {
        assert_eq!(resolve_href("OEBPS/text/ch01.html", "https://example.com/img.png"), "");
    }
    ...
    fn sentinel_survives_root_level_img() {
        ...
        let text = html2text::from_read(&html[..], 80).unwrap();
        assert!(text.contains("__INKIMG_0__"), "sentinel must survive: {:?}", text);
    }
}
